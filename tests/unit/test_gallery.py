...
because two implementations of the same arithmetic have to agree.

**That agreement is what these tests protect.** The mirrored assertions live in
the browser tier, which checks the ids actually rendered
(`the page shows images 1 to 10`). If those two ever diverge, tiles will request
ids the server considers out of range.
"""
...
def test_pages_do_not_overlap_and_leave_no_gaps():
    ...
    happens to be right at the boundaries tested above.
    """
...
def test_ranges_follow_the_chosen_count():
    assert ids(1, count=20) == list(range(1, 21))
    assert ids(2, count=20) == list(range(21, 41))
    assert ids(2, count=50) == list(range(51, 101))
